...
#include "MyAllocator.h" // base class allocator
#include "IO.h"
...
class Benchmark {
 public:
  ...
  void SingleAllocation(Allocator<int>* allocator, const std::size_t size);/*
  void SingleFree(Allocator<int>* allocator, const std::size_t size);
...
*/
 private:
  ...
  const BenchmarkResults buildResults(std::size_t nOperations, std::chrono::milliseconds&& ellapsedTime) const;
